...
Svgmule.prototype._processFile = function(file, opt_config_) {
	...
	var considerFileConfig = function(file_, conf) {
		var file = path.join( path.dirname(file_), path.basename(file_, '.svg')+'.yml' );
...
Svgmule.prototype._processFiles = function(files, dir) {
	...
	var considerDirConfig = function(dir, conf) {
		var dir_name    = _.str.trim(dir, path.sep).split(path.sep).pop();
		var file_name   = dir_name+'.yml';
		var config_file = path.join(dir, file_name);

		return _this._loadOptimizerConfig(config_file, conf);
	};
